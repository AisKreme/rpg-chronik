...
/**
 * Speichert oder aktualisiert einen Monster-Eintrag mit Bildern
 */
export async function saveMonsterEntry({ name, beschreibung, werte, images, ort, editMonsterId }) {
  const payload = {
    name,
    beschreibung,
    werte,
  ...
  const newId = await saveEntryWithImages(
    'monsters',         // Tabellenname
    payload,
    images,
    editMonsterId,
    'monsters'          // Storage-Bucket
  )
